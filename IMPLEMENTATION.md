...
The current generator already covers the rounded-rectangle family, including:
...
- or generate a custom preset from the CLI
...
- `reflectGlassCircle` for the circular preset
- `reflectGlassRoundedSquare` for the rounded-square preset
...
For multi-button pages, each button can point at its own anchor through:

```html
<button data-reflect-glass data-reflect-glass-anchor="#circle-anchor"></button>
```
